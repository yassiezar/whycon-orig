			...
			if (vd->toggleAvi) {
				...
				if (vd->avifile == NULL) {
					...
					if (vd->avifile == NULL ) {
						...
						AVI_set_video(vd->avifile, vd->width, vd->height,vd->fps, "MJPG");
						printf("recording to %s\n",vd->avifilename);
					}
				} else {
					/* if we have a valid avifile, record the frame to it - not working, removed*/
					if (AVI_write_frame(vd->avifile, vd->tmpbuffer,vd->buf.bytesused, vd->framecount)!=0)
					{
						AVI_close(vd->avifile);
						sprintf(vd->avifilename,"%s.avi",vd->avifilename);
						vd->avifile = AVI_open_output_file(vd->avifilename);
						AVI_set_video(vd->avifile, vd->width, vd->height,vd->fps, "MJPG");
					}
					vd->framecount++;
